...
// $Id: NumiTargetHall.cc,v 1.11.2.9 2010/03/16 16:41:36 corwin Exp $
//----------------------------------------------------------------------
...
//TGAR
  ...
  if (NumiData->NTHConcreteSectionsN!=0){
    ...
    tempgnumiD = new G4Box("sDuratekgnumiD", 0.5842*m,0.301475*m, (7.85+4.5)*m); // Needs to be 9m longer (4.5m in G4Box since it adds 4.5m to each end). and moved by +4.5m in z
    G4LogicalVolume *lvDuratekBlockgnumiD = new G4LogicalVolume(tempgnumiD, Fe, "lvDuratekBlockgnumiD", 0, 0,0);
    //Move this shielding block 9m downstream to account for new position of horn2, change lenght of D and D2 accordingly.
    ...
    tempgnumiD2 = new G4Box("sDuratekgnumiD2", 0.5842*m, 0.301475*m,(15.349-4.5)*m); // Needs to be 9m shorter (4.5m in G4Box since it adds 4.5m to each end). and moved by +4.5m in z
